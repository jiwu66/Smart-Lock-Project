...
int main(void){
	...
	while(1)
	{
		...
		key = key_scan();
		if(key == 1)//If unlock has been selected, run FP_unlock
		{
			OLED_Clear();
			
			FP_unlock();
		}
		else if(key == 2)//If Admi Mode selected
		{
			OLED_Clear();
			OLED_ShowStr(0,0,"Enter Password:");//Ask for password to enter admin mode
			...
			if(stat == 1)
			{
				...
				{
					key = key_scan();
					if(key == 1)//If ""Add FP"" Selected, run Record_FP
					{
						OLED_Clear();
						Record_FP();
						break;
					}
					else if(key == 2)//If "Delete FPs"" selected, run FP_Delete
					{
						OLED_Clear();
						FP_Delete();
						break;
					}
					else if(key == 3)//If "Change Password" selected, run Change Password
					{
